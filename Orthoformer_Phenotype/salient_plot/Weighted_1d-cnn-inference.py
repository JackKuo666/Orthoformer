...
def get_important_OG(saliency_list, original_seq_list, sample_name_list, top_k=10):
    """
    Find the top_k OGs with the highest contribution for each sample based on saliency.

    Parameters
    ----
    saliency_list : list of np.ndarray
        Saliency vector for each sample (length = number of effective positions).
    ...
    top_k : int
        Number of top K positions to output for each sample (sorted by saliency in descending order).

    Returns
    ----
    all_top_positions : list[list[int]]
        List of top_k position indices for each sample (sorted by saliency in descending order).
    ...
    for sal_i, og_list, name in zip(saliency_list, original_seq_list, sample_name_list):
        ...
        # Take the top_k positions, sorted by saliency in descending order
        k = min(top_k, L_eff)
...
def plot_sequence_saliency_heatmap(
    ...
    """
    sequence: list(str) or str, length L (OG sequence here)
...
def train_1dcnn(args):
    ...
    if class_sample_count.size < 2:
        # Extreme case: only one class in training set, fall back to normal shuffle
        ...
        # Give larger sampling weights to minority class
    ...
    # ===== Loss: Use FocalLoss (for class imbalance) =====
    pos_frac = y.mean()
    alpha = 1.0 - pos_frac
    ...
    min_delta = 1e-3  # Minimum AUC improvement required to be considered as "improvement"
    no_improve_count = 0
    # ==============================================
    ...
    for epoch in range(num_epochs):
        ...
        # ===== Scan thresholds on validation set, compute balanced accuracy to mitigate extreme imbalance =====
        ...
        # Log output: include AUC, acc@0.5, and balanced accuracy at best threshold
        if auc is not None:
    ...
    # ===== Overwrite model parameters with best checkpoint =====
    ...
    # Output top-K OGs for each sample (K controlled by --top_k parameter)
    ...
    # Save top-K results for all samples to CSV for subsequent analysis
    if len(records) > 0:
    ...
    # If you want to view the heatmap for a specific sample:
