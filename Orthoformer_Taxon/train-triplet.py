...
def parse():
    ...
    parser.add_argument('-n', '--model_name', type=str, default='train_aug2_species_s20000_taxid')
    parser.add_argument('-t', '--training_data', type=str, default='train_aug2_species_s20000_taxid')
    parser.add_argument('-d', '--hidden_dim', type=int, default=384)
...
def get_dataloader(dist_map, id_ec, ec_id, args):
    ...
    negative = mine_hard_negative_cuda(dist_map, 30)
...
def main():
    ...
    # esm_emb = pickle.load(
    #     open('/mnt/disk_c/user_data/liuping/gtdk/distance_map/' + args.training_data + '_esm.pkl',
    #             'rb')).to(device=device, dtype=dtype)
    # dist_map = pickle.load(open('/mnt/disk_c/user_data/liuping/gtdk/distance_map/' + \
    # args.training_data + '.pkl', 'rb')) 
    # List all the chunk files
    data_dir = './data/distance_map/'
    loaded_esm_dist = {}
    esm_dist_chunk_files = [f for f in os.listdir(data_dir) if f.startswith(f'{args.training_data}_esm_dist_chunk')]
     # Sort the chunk files by the index to ensure the correct order
    esm_dist_chunk_files.sort(key=lambda x: int(x.split('_')[-1].split('.pt')[0]))  # Extract index from file name
    for chunk_file in esm_dist_chunk_files:
        chunk_path = os.path.join(data_dir, chunk_file)
        if os.path.exists(chunk_path):
            chunk_dict = torch.load(chunk_path)
            loaded_esm_dist.update(chunk_dict)
            print(f"Loaded {chunk_file} with {len(chunk_dict)} keys.")
    # total_chunks = 5
    # Find all chunk files (assuming you know or pattern match them)
    # for i in range(1, total_chunks + 1):
    #     chunk_path = os.path.join(data_dir, f'{args.training_data}_esm_dist_chunk_{i}.pt')
    #     if os.path.exists(chunk_path):
    #         chunk_dict = torch.load(chunk_path)
    #         loaded_esm_dist.update(chunk_dict)
    #         print(f"Loaded {args.training_data}_esm_dist_chunk_{i}.pt with {len(chunk_dict)} keys.")

    print(f"Total keys after loading all chunks: {len(loaded_esm_dist)}")
    dist_map = loaded_esm_dist
    chunk_files = [f for f in os.listdir(data_dir) if f.startswith(f'{args.training_data}_chunk')]

    # Sort the chunk files by the index to ensure the correct order
    chunk_files.sort(key=lambda x: int(x.split('_')[-1].split('.pt')[0]))  # Extract index from file name

    # Initialize an empty list to store the loaded chunks
    chunks = []
    ...
    esm_emb = torch.cat(chunks, dim=0)

    print(f"Merged tensor shape: {esm_emb.shape}")
    # Open the gzipped pickle file and load the data
    # with gzip.open('/mnt/disk_c/user_data/liuping/gtdk/distance_map/' + \
    #  args.training_data + '.pkl.gz', 'rb') as f:
