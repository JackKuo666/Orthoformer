...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Feature extraction example with ALiBi positional encoding support",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
        "--model_dir",
        type=str,
        default="/mnt/MAG/foundation_model/gene_bert_output/model_3M_2048_v8/final_model/",
        help="Path to model directory"
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default="datasets/JGI_IMG_isolate_batch_1.dataset",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Use ALiBi positional encoding (default: True, use --no_alibi to disable)"
    )
    parser.add_argument(
        "--no_alibi",
        dest="use_alibi",
        action="store_false",
        help="Disable ALiBi positional encoding"
    )
...
def load_model_with_alibi(model_dir, device, use_alibi=True):
    ...
    # Check training config: if ALiBi was used, need to manually replace attention layers
...
def print_model_info(model):
    ...
    # Note: Token Type Embeddings is actually [1, 512], not [2, 512]
...
def process_samples(model, tokenizer, dataset, device, num_samples, max_length):
    """Process samples and generate embeddings"""
    ...
    if "input_ids" in samples:
        ...
        max_seq_length = max(len(seq) for seq in samples["input_ids"])
        
        model_max_length = model.config.max_position_embeddings
        # Limit max length to avoid memory issues
        max_seq_length = min(max_seq_length, model_max_length)
        ...
        for seq in samples["input_ids"]:
            if len(seq) > max_seq_length:
                ...
                # Pad
                padded_seq = seq + [tokenizer.pad_token_id] * (max_seq_length - len(seq))
    ...
    with torch.no_grad():
        ...
        return hidden_states, attention_mask, model.config

def process_attention_pooling(hidden_states, attention_mask, model_dir, device):
    """Process attention pooling using classification head"""
    ...
    if os.path.exists(attn_head_path):
        try:
            ...
            if "attn_vec" in state:
                ...
                # Optional: Apply L2 normalization to sentence vectors for more stable retrieval/clustering
                ...
                # Brief comparison and usage recommendations for sentence vector construction methods
...
def main():
    ...
    if torch.cuda.is_available() and args.device_id >= 0:
        ...
        print(f"GPU Memory: {torch.cuda.get_device_properties(args.device_id).total_memory / 1024**3:.1f} GB")
    
    # Load trained model and tokenizer
    model, tokenizer = load_model_with_alibi(args.model_dir, device, use_alibi=args.use_alibi)
    ...
    # Process samples and generate embeddings
    hidden_states, attention_mask, _ = process_samples(
        model, tokenizer, dataset, device, args.num_samples, args.max_length
    )
    
    # Process attention pooling if available
    process_attention_pooling(hidden_states, attention_mask, args.model_dir, device)

if __name__ == "__main__":
    main()

""" Run Example:
python feature_extraction_example_alibi.py \
